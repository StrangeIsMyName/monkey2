...
Class TextDocument
	...
	Method New()
		...
		Code.NodeClicked = Lambda( event:MouseEvent )
	
			CodeClicked( Code.SelectedLine, Code.SelectedLabel )
...
'				print "1 "+line+" "+jumpline
			...
			jumpline = line'_lines.Data[line].line
...
'	Field DebugClicked:Void()
	...
	method GetCursorForward:bool()
		Return (_cursorLocation < _cursorPositionCount) And _cursorPositionCount > 0
	End
	
	
	method GetCursorBack:bool()
		Return _cursorLocation > 0
	End
...
'      print " cursor clicked click="+_clickedCount
 ...
 '           print " eat space"
	...
	Method GetPrevWord:string( line:int )
		If line <= 0 or line > _doc.LineCount Return ""
		...
		if alpha.Find( chr ) = -1 then 'next non alpha
			...
			cursor -= pos
		Endif
		Return text.Mid(cursor, _cursor - cursor )
	End Method
