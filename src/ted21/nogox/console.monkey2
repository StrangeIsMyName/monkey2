...
Class Console Extends TextView
	...
	Method Start:Bool( cmd:String )
		...
		_process.StdoutReady = Lambda()
			...
			If Not stdout then
				If _stdout then _stdoutBuf.Add( _stdout )
				_stdoutOpen = False
				UpdateRunning()
				Return
			Endif
			...
			If _stdoutWaiting And Not _stdoutBuf.Empty then _stdoutWaiting.Set( True )
			
		End

		'print "3 "+cmd
		
		If Not _process.Start( cmd ) then Return False
