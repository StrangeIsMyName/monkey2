...
Class DebugView Extends DockingView
  ...
  Field _debugging:bool
	...
	Method New()
		If Not icons[0] Then
			'DebugStop()
			icons = Theme.LoadIcons( "asset::ted21/debug_icons.png",16 )
		End If
	...
	Method UpdateExpanded( node:Node )
		...
		For Local i:=0 Until lines.Length
			...
			Else
				New Node( line, NODEKIND_DEBUG, node )
			Endif
	...
	Method UpdateTree()
		...
		Repeat
			...
			If line.StartsWith( ">" )
				...
				Else
					func = New Node( label+"*", NODEKIND_DEBUGFUNCTION, root,seq,funcIndex )
					func.srcFile = bits[1]
					func.srcLine = Int( bits[2] )
					func.seq = seq
				...
				If Not first then
					...
					If doc Then doc.DebugLine = func.srcLine-1
					first = func
				Endif
        ...
        Local kind:Int = NODEKIND_DEBUG
        
				If line.Left(5) = "Self:" Then
          line = line.Right( line.Length - 5 )
          line = line.Left( line.Length - 16 )
          line = "Parent = " + line
          kind = NODEKIND_DEBUGPARENT
        End if
        If line.Contains( ":Int" ) Or line.Contains( ":Float" ) Or line.Contains( ":Byte" ) Or line.Contains( ":Double" ) Or line.Contains( ":Long" ) then
          kind = NODEKIND_DEBUGVAR
        End if
        if line.Contains( ":Short" ) Or line.Contains( ":UInt" ) Or line.Contains( ":UFloat" ) Or line.Contains( ":UByte" ) Or line.Contains( ":UDouble" ) then
          kind = NODEKIND_DEBUGVAR
        End if
        If line.Contains( ":ULong" ) Or line.Contains( ":UShort" ) then
          kind = NODEKIND_DEBUGVAR
				End If

				New Node( line, kind, func )
...
Private
	...
	Class Node Extends TreeView.Node
		...
		Method New( label:String, kind:Int, parent:TreeView.Node = Null, seq:Int = 0, index:Int = -1 )
			Super.New( "", kind, parent, index )
			Self.seq = seq

        select kind
          case NODEKIND_DEBUGPARENT, NODEKIND_DEBUGVAR, NODEKIND_DEBUG
            Index = -1
        end Select
...
'							label+=" (null)"
			...
			If scope New Node( "", NODEKIND_DEBUG, Self )
		End
